...
class TestPy2Dict(BaseMapping):
    ...
    def test_keys(self):
        """ .keys method """
        self.assertIsInstance(list(self.f.keys()), list)
        self.assertSameElements(list(self.f.keys()), self.groups)

    def test_values(self):
        """ .values method """
        self.assertIsInstance(list(self.f.values()), list)
        self.assertSameElements(list(self.f.values()), [self.f.get(x) for x in self.groups])

    def test_items(self):
        """ .items method """
        self.assertIsInstance(list(self.f.items()), list)
        self.assertSameElements(list(self.f.items()),
            [(x, self.f.get(x)) for x in self.groups])

    def test_iterkeys(self):
        """ .iterkeys method """
        self.assertSameElements([x for x in self.f.keys()], self.groups)

    def test_itervalues(self):
        """ .itervalues method """
        self.assertSameElements([x for x in self.f.values()],
            [self.f.get(x) for x in self.groups])

    def test_iteritems(self):
        """ .iteritems method """
        self.assertSameElements([x for x in self.f.items()],
            [(x, self.f.get(x)) for x in self.groups])
...
class TestCopy(TestCase):
    ...
    def test_copy_without_attributes(self):
        ...
        self.assertArrayEqual(self.f1['baz'], np.array([1,2,3]))
        self.assertTrue('bar' not in self.f1['baz'].attrs)

        self.f2.copy(foo, 'baz', without_attrs=True)
        self.assertArrayEqual(self.f2['baz'], np.array([1,2,3]))
        self.assertTrue('bar' not in self.f2['baz'].attrs)
